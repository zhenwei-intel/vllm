...
def compute_sub_block_ptrs(
    ...
    block_ptr_dtype = np.uint64 if current_platform.is_xpu() else np.int64

    if block_size_factor == 1:
        # Fast path: 1:1 mapping, no sub-block expansion needed.
        output[:] = (
            base_ptr + block_ids[:num_sub_blocks].astype(block_ptr_dtype) * row_stride
        )
        return
    ...
    sub_offsets = np.arange(block_size_factor, dtype=block_ptr_dtype) * sub_block_size
    # (num_blocks, 1) + (1, block_size_factor) -> (num_blocks, block_size_factor)
    all_ptrs = (
        base_ptr + block_ids.astype(block_ptr_dtype)[:, np.newaxis] * row_stride
...
class SingleDirectionOffloadingHandler(OffloadingHandler):
    ...
    def __init__(
        ...
        # XPU memory addresses can exceed int64 range, so use uint64 for pointer
        # arithmetic.
        self.block_ptr_dtype = np.uint64 if current_platform.is_xpu() else np.int64

        # Pre-compute block sizes for batch copies.
        self._block_size_in_bytes_arr = np.array(
            self.tensor_block_size_in_bytes, dtype=self.block_ptr_dtype
        )
    ...
    def transfer_async(self, job_id: int, transfer_spec: TransferSpec) -> bool:
        ...
        all_src = np.empty(total, dtype=self.block_ptr_dtype)
        all_dst = np.empty(total, dtype=self.block_ptr_dtype)
        all_sizes = np.empty(total, dtype=self.block_ptr_dtype)
